...
#The output file is "$filename(big/small).in" and "AMIE.txt"(au, mass, inclination, eccentricity) for plot.
#This script requires a "init.txt" file,
#There mustn't be "$filename(big/small).in" or "AMIE.txt" in the directory before you run it.
...
then
	echo " 'AMIE.txt' already exist! Please move it to another directory!"
	exit 1
fi
	...
	END {
		...
		while (Embro <= '"$bodynumber"' ) {
			...
			if (AuX[Embro] < '"$snowline"')
				DenX=rand()*is_density_range+'"$is_density_begin"'
			else 
				DenX=rand()*('"$os_density_end"'-'"$os_density_begin"')+'"$os_density_begin"'
			if (Embro < 10) 
				Prefix = "EM0000"
			else if (Embro < 100)
				Prefix = "EM000"
			...
			printf"%12.11E %12.11E %12.11E %12.11E\n", AuX[Embro], MassX, Incli, Eccen >> "AMIE.txt"
			++Embro
			}
	}
	' nuau
...
############################## show total mass
TotEmMass=`awk '{sum+=$2}; END{print sum}' AMIE.txt`
